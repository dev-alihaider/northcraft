...
const ProjectsShowcase: React.FC = () => {
    ...
    const handleButtonClick = (label: string) => {
        ...
        if (label === "All Works") {
            ...
            const filteredImages = carouselsImages.filter((image) => image.category === label);
    ...
    return (
        <div className="bg-blue py-8">
            <div className="flex sm:flex-row flex-col items-center sm:pt-34 py-2 large:px-28 lg:px-20 sm:px-12 px-6">
                <div
                    className=" font-semibold large:text-5xl text-3xl sm:pl-12 sm:pb-0 pb-2 sm:text-left text-center sm:w-1/3 ">
                    Featured Project
                </div>
                <div
                    className=" flex gap-2 sm:ml-auto pl-4 font-semibold large:text-lg text-base sm:text-left justify-center text-center text-white flex-wrap">
                    {buttons.map((label, i: number) => (<Button
                        key={i}
            ...
            <div className="lg:px-28 sm:px-16 ">
                <div
                    className="sm:grid hidden pb-4 lg:px-28 sm:px-16 sm:grid-cols-4 grid-cols-1 sm:gap-6 pt-10 sm:pb-2 px-4 text-darkGray font-medium lg:text-xl text-sm sm:overflow-auto overflow-hidden ">
